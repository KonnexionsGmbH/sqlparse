...
fold_tree(_Ind, _Idx, _Parent, {_,<<>>}, _Fun, Acc) -> Acc;									%% pre-order empty hints or opt
...
fold_tree(Ind, Idx, Parent, [{'fun', Name, Parameters}|Rest], Fun, Acc0) ->					%% pre-order to function in-order transition
	NewInd = Ind+1,
	Acc1 = fold_tree(NewInd, 0, Name, {'fun', Name, Parameters}, Fun, Acc0),	
	fold_tree(Ind, Idx+1, Parent, Rest, Fun, Acc1);			

fold_tree(Ind, _Idx, Parent, {'fun', Name, Parameters}, Fun, Acc0) -> 						%% function evaluation
	io:format(user, "~n~p,~p,~p,~p", [Ind, 0, Parent, Name]),
	Acc1 = Fun(Ind, 0, Parent, Parameters, visit, Name, Acc0),		
