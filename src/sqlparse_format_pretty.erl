...
fold(LOpts, FunState, Ctx, _PTree, {procedure_call, Step, Pos} =
    _FoldState) ->
    ?CUSTOM_INIT(FunState, Ctx, _PTree, _FoldState),
    RT = case {Step, Pos} of
             {start, _} -> Ctx ++ format_new_statement(LOpts, FunState, []);
             {'end', other} -> Ctx ++ ";";
             _ -> Ctx
         end,
    ?CUSTOM_RESULT(RT);
...
fold(LOpts, _FunState, Ctx, {table_coll_expr, CollExpr, Optional} = _PTree,
...
fold(LOpts, _FunState, Ctx, {table_coll_expr, _CollExpr, Optional} = _PTree,
    ...
    ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
    RT = case {Step, Optional} of
             {start, _} -> Ctx ++ format_keyword(LOpts, "table (");
             {'end', '+'} -> Ctx ++ ") (+)";
             _ -> Ctx ++ ")"
         end,
    ?CUSTOM_RESULT(RT);
