...
<<"SELECT SED_ORDER, SED_CHARGE, SED_ETID, SED_DESC, SED_TARID, SED_TOTAL,\nSED_COUNT1, SED_COUNT2, SED_ESID, SED_INT, SED_AMOUNTCU, SED_RETSHAREPV,\nSED_RETSHAREMO, SED_PREPAID, SED_LONGID, SED_UFIH_STREAM, SED_UFIH_BOSID,\nSED_UFIH_BOHID, SED_UFIH_OUT\nFROM SETDETAIL\nWHERE SED_ORDER Like\nto_char(sysdate,'YYYY-MM') || '%'">>,
 ...
 <<"SELECT CON_CONSOL, AC_NAME, CON_NAME, CON_TARID, A_MAIN.ADR_EMAIL,\nA_MAINCONTACT.ADR_EMAIL, A_MARKETING.ADR_EMAIL, A_TECHNICAL.ADR_EMAIL,\nA_HELPDESK.ADR_EMAIL, A_FINANCE.ADR_EMAIL, A_OTHERS.ADR_EMAIL,\nA_INVOICE.ADR_EMAIL\nFROM CONTRACT, ACOUNT, ADDRESS A_MAIN, ADDRESS\nA_MAINCONTACT, ADDRESS A_MARKETING, ADDRESS A_TECHNICAL, ADDRESS A_HELPDESK,\nADDRESS A_FINANCE, ADDRESS A_OTHERS, ADDRESS A_INVOICE\nWHERE CONTRACT.CON_ACID\n= ACOUNT.AC_ID AND A_MAIN.ADR_ID = ACOUNT.AC_ADRID_MAIN AND\nA_MAINCONTACT.ADR_ID = ACOUNT.AC_ADRID_MAINCONTACT AND A_MARKETING.ADR_ID =\nACOUNT.AC_ADRID_MARKETING AND A_TECHNICAL.ADR_ID = ACOUNT.AC_ADRID_TECHNICAL\nAND A_HELPDESK.ADR_ID = ACOUNT.AC_ADRID_HELPDESK AND A_FINANCE.ADR_ID =\nACOUNT.AC_ADRID_FINANCE AND A_OTHERS.ADR_ID = ACOUNT.AC_ADRID_OTHERS AND\nA_INVOICE.ADR_ID = ACOUNT.AC_ADRID_INVOICE AND CON_ETID = 'LAC' AND CON_ESID\nIn ('A','E')\nORDER BY to_number(CON_CONSOL)">>,
...
<<"select USERNAME from ALL_USERS where username Like '%YS' and username NOT Like '%YS' or username Like '%YS' ESCAPE 'abcd' and not (username Like '%YS' ESCAPE 'abcd')">>,
