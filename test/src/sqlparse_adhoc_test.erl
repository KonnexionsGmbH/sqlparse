        ...
        case ?TEST_VERSION of
            ...
            check ->
                case sqlparse_fold:top_down(sqlparse_check_td_vs_bu, ParseTree,
                    top_down) of
                    {error, _} = TD -> TD;
                    TD -> case sqlparse_fold:bottom_up(sqlparse_check_td_vs_bu,
                        ParseTree, bottom_up) of
                              {error, _} = BU -> BU;
                              BU ->
                                  io:format(user, "~n", []),
                                  io:format(user, ?MODULE_STRING ++
            ...
            full ->
                sqlparse_test_utils:eunit_test(Source);
            pretty ->
                sqlparse_fold:top_down(sqlparse_format_pretty, ParseTree,
                    ?LOPTS);
            top_down ->
                case sqlparse_fold:top_down(sqlparse_check_td_vs_bu, ParseTree,
                    top_down) of
