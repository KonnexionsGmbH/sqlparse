    ...
    {ok, {ParseTree, Tokens}} ->
      ...
      Source_TD =
        case sqlparse_fold:top_down(sqlparse_format_flat, ParseTree, []) of
          {error, Error_1_TD} ->
            ct:pal("[TD] Error ParseTree ==> Source_TD : Error    ~n > ~p~n", [Error_1_TD]),
            ct:pal("[TD] Error ParseTree ==> Source_TD : Source   ~n > ~p~n", [Source]),
            ct:pal("[TD] Error ParseTree ==> Source_TD : ParseTree~n > ~p~n", [ParseTree]),
      ...
      {ok, {ParseTree_TD, Tokens_TD}} =
        try
          ...
          Exception_TD:Reason_TD ->
            ct:pal("[TD] Error Source_TD ==> ParseTree_TD : Exception~n > ~p~n", [Exception_TD]),
            ct:pal("[TD] Error Source_TD ==> ParseTree_TD : Reason   ~n > ~p~n", [Reason_TD]),
            ct:pal("[TD] Error Source_TD ==> ParseTree_TD : Source   ~n > ~p~n", [Source]),
            ct:pal("[TD] Error Source_TD ==> ParseTree_TD : Source_TD~n > ~p~n", [Source_TD])
        end,
      %% -----------------------------------------------------------------
      ...
      if
        ParseTree /= ParseTree_TD ->
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : Source      ~n > ~p~n", [Source]),
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : Source_TD   ~n > ~p~n", [Source_TD]),
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : ParseTree   ~n > ~p~n", [ParseTree]),
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : ParseTree_TD~n > ~p~n", [ParseTree_TD]),
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : Tokens      ~n > ~p~n", [Tokens]),
          ct:pal("[TD] Error ParseTree /= ParseTree_TD : Tokens_TD   ~n > ~p~n", [Tokens_TD]),
      ...
      case Source_TD_MultipleSpace of
        ...
        _ ->
          ...
          ct:pal("[TD] Error redundant whitespace(s) : Source         ~n > ~p~n", [Source]),
          ct:pal("[TD] Error redundant whitespace(s) : Source_TD      ~n > ~p~n", [Source_TD]),
      ...
      Source_Check_TD =
        case sqlparse_fold:top_down(sqlparse_check_td_vs_bu, ParseTree, top_down) of
          {error, Error_Check_TD} ->
            ct:pal("[TD==BU] Error ParseTree ==> Check_TD : Error    ~n > ~p", [Error_Check_TD]),
            ct:pal("[TD==BU] Error ParseTree ==> Check_TD : Source   ~n > ~p", [Source]),
            ct:pal("[TD==BU] Error ParseTree ==> Check_TD : ParseTree~n > ~p", [ParseTree]),
      ...
      Source_Check_BU =
        case sqlparse_fold:bottom_up(sqlparse_check_td_vs_bu, ParseTree, bottom_up) of
          {error, Error_Check_BU} ->
            ct:pal("[TD==BU] Error ParseTree ==> Check_BU : Error    ~n > ~p", [Error_Check_BU]),
            ct:pal("[TD==BU] Error ParseTree ==> Check_BU : Source   ~n > ~p", [Source]),
            ct:pal("[TD==BU] Error ParseTree ==> Check_BU : ParseTree~n > ~p", [ParseTree]),
      ...
      if
        Source_Check_TD /= Source_Check_BU ->
          ct:pal("[TD==BU] Error Source_Check_TD /= Source_Check_BU : Source  ~n > ~p", [Source]),
          ct:pal(
      ...
      %% -----------------------------------------------------------------
      Source_FORMAT =
        case sqlparse_fold:top_down(sqlparse_format_pretty, ParseTree, []) of
          {error, Error_1_FORMAT} ->
            ct:pal(
              "[FORMAT] Error ParseTree ==> Source_FORMAT : Error    ~n > ~p~n",
              [Error_1_FORMAT]
            ),
            ct:pal("[FORMAT] Error ParseTree ==> Source_FORMAT : Source   ~n > ~p~n", [Source]),
            ct:pal("[FORMAT] Error ParseTree ==> Source_FORMAT : ParseTree~n > ~p~n", [ParseTree]),
      ...
      if
        ParseTreeLower /= ParseTree_FORMATLower ->
          ct:pal("[FORMAT] Error ParseTree /= ParseTree_FORMAT : Source      ~n > ~p~n", [Source]),
          ct:pal(
          ...
          ct:pal("[FORMAT] Error ParseTree /= ParseTree_FORMAT : Tokens      ~n > ~p~n", [Tokens]),
          ct:pal(
...
eunit_test_() ->
  ?D("Start~n", []),
  WCard =
    case os:getenv(?ENV_VAR_FILE_WILDCARD) of
      SourceFiles when is_list(SourceFiles) -> SourceFiles;
      _ -> "*"
...
eunit_test_source(TestGroup, Source) ->
  ...
  case sqlparse_test_utils:eunit_test(Source) of
    ...
    Result ->
      io:format(
        user,
        "~n" ++ ?MODULE_STRING ++ " : Error in eunit_test_source : ErrorResult~n > ~p~n",
        [Result]
      ),
      io:format(
        user,
        "~n" ++ ?MODULE_STRING ++ " : Error in eunit_test_source : TestGroup  ~n > ~p~n",
        [TestGroup]
      )
  end.
      ...
      () ->
        case TestFiles of
          ...
          [TestFile | RestTestFiles] ->
            ...
            TestGroup = filename:rootname(filename:basename(TestFile)),
            [tests_gen(TestGroup, AugTests, Opts) | group_gen(RestTestFiles)]
        end
    end
  }.
...
tests_gen(TestGroup, Tests, Opts) ->
  ?D("Start~n TestGroup: ~p~n Tests: ~p~n Opts: ~p~n", [TestGroup, Tests, Opts]),
  SelTests =
...
tests_gen(_TestGroup, [], _SelTests, Acc) -> {inorder, lists:reverse(Acc)};
  ...
  case lists:member(I, SelTests) of
    true ->
      tests_gen(
        ...
        SelTests,
        [
          {TestGroup, I, fun () -> {timeout, ?TIMEOUT, ?MODULE:eunit_test_source(TestGroup, T)} end}
          | Acc
        ]
      );
