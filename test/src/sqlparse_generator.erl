...
generate() ->
  ets:new(?CODE_TEMPLATES, [named_table, private, ordered_set]),
  create_code(),
  %% Common tests ............................................................
  case ?GENERATE_CT of
    true ->
      ...
      case ?GENERATE_RELIABILITY of
        true ->
          case ?GENERATE_COMPACTED of
            true ->
              ok =
                file_create_ct_all("reliability", "complete_", "compacted", ?ALL_CLAUSE_RELIABILITY),
              ok =
                file_create_ct_all(
                  "reliability",
                  "beginend_",
            ...
            _ ->
              ok =
                file_create_ct_all("reliability", "complete_", "detailed_", ?ALL_CLAUSE_RELIABILITY),
              ok =
                file_create_ct_all(
                  "reliability",
                  "beginend_",
    ...
    true ->
      case ?GENERATE_RELIABILITY of
        true ->
          ...
          ok = file_create_eunit_all("reliability", "beginend_", ?ALL_CLAUSE_RELIABILITY_PLSQL),
          ...
          ok =
            file_create_eunit_all("reliability", "complete_", ?ALL_CLAUSE_RELIABILITY_SQL_DETAILED);
          ...
          case rand:uniform(3) rem 3 of
              ...
              lists:append(
                [
                  case rand:uniform(3) rem 3 of
                    ...
                    2 ->
                      lists:append([",", lists:nth(rand:uniform(Identifier_Length), Identifier)]);
            ...
            2 ->
              case rand:uniform(2) rem 2 of
                ...
                _ -> lists:append([" ", lists:nth(rand:uniform(Spec_Item_Length), Spec_Item)])
              end;
...
create_code(assignment_statement = Rule) ->
  ...
  [{scalar_opt_as_exp_1, Scalar_Opt_As_Exp_1}] = ets:lookup(?CODE_TEMPLATES, scalar_opt_as_exp_1),
    ...
    [
      lists:append(
        [
          ...
          " := ",
          lists:nth(rand:uniform(Scalar_Opt_As_Exp_1_Length), Scalar_Opt_As_Exp_1)
        ]
      )
      || _ <- lists:seq(1, ?MAX_STATEMENT_SIMPLE * 2)
    ],
    ...
    [
      lists:append(
        [
          "Case",
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Scalar_Opt_As_Exp_Length), Scalar_Opt_As_Exp);
            _ -> []
          end,
          " ",
          lists:nth(rand:uniform(Case_When_Then_List_Length), Case_When_Then_List),
          case rand:uniform(2) rem 2 of
            1 -> " Else " ++ lists:nth(rand:uniform(Scalar_Opt_As_Exp_Length), Scalar_Opt_As_Exp);
            _ -> []
          end,
          " End"
        ]
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ -> lists:nth(rand:uniform(Case_When_Then_Length), Case_When_Then)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
      ...
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            ...
            2 -> lists:nth(rand:uniform(Column_Def_Opt_Length), Column_Def_Opt);
            _ -> []
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
  ...
  Code =
    ...
    [
      case rand:uniform(3) rem 3 of
        ...
        2 ->
          lists:append(
            ["Check (", lists:nth(rand:uniform(Search_Condition_Length), Search_Condition), ")"]
          );
          ...
          lists:append(
            [
              case rand:uniform(2) rem 2 of
                ...
                _ -> lists:nth(rand:uniform(Function_Ref_Length), Function_Ref)
              end,
              ",",
              case rand:uniform(2) rem 2 of
                1 -> lists:nth(rand:uniform(Column_Ref_Length), Column_Ref);
...
create_code(create_role_def = Rule) ->
  ?CREATE_CODE_START,
  [{identifier, Identifier}] = ets:lookup(?CODE_TEMPLATES, identifier),
  Code = ["Create Role " ++ N || N <- Identifier],
  store_code(Rule, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(manipulative_statement, Code, ?MAX_STATEMENT_SIMPLE, true),
  ?CREATE_CODE_END;
      ...
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            ...
            2 -> lists:nth(rand:uniform(Base_Table_Element_Length), Base_Table_Element);
            _ -> []
          end,
          ")"
        ]
      ...
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            ...
            2 -> " " ++ lists:nth(rand:uniform(User_Opt_Length), User_Opt);
            _ -> []
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_STATEMENT_SIMPLE * 2)
...
create_code(cursor = Rule) ->
  ?CREATE_CODE_START,
  [{identifier, Identifier}] = ets:lookup(?CODE_TEMPLATES, identifier),
  Code_1 = [re:replace(N, "ident", "cursor", [{return, list}]) || N <- Identifier],
  Code = [re:replace(N, "IDENT", "CURSOR", [{return, list}]) || N <- Code_1],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
  ...
  Code =
    ["Authentication Required"] ++ [
      case rand:uniform(2) rem 2 of
        ...
        _ -> lists:nth(rand:uniform(Proxy_With_Length), Proxy_With) ++ " Authentication Required"
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            1 -> " Where Current Of " ++ lists:nth(rand:uniform(Cursor_Length), Cursor);
            _ -> " " ++ lists:nth(rand:uniform(Where_Clause_Length), Where_Clause)
          end,
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Returning_Length), Returning);
            _ -> []
          end
        ]
...
create_code(drop_context_def = Rule) ->
  ...
  Code = ["Drop Context " ++ N || N <- Context_Name],
  store_code(Rule, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(manipulative_statement, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(sql, Code, ?MAX_STATEMENT_SIMPLE, true),
  ?CREATE_CODE_END;
...
create_code(drop_directory_def = Rule) ->
  ?CREATE_CODE_START,
  [{directory_name, Directory_Name}] = ets:lookup(?CODE_TEMPLATES, directory_name),
  Code = ["Drop Directory " ++ N || N <- Directory_Name],
  store_code(Rule, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(manipulative_statement, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(sql, Code, ?MAX_STATEMENT_SIMPLE, true),
  ?CREATE_CODE_END;
...
create_code(drop_function_def = Rule) ->
  ?CREATE_CODE_START,
  [{function_name, Function_Name}] = ets:lookup(?CODE_TEMPLATES, function_name),
...
create_code(drop_index_def = Rule) ->
  ...
  Code =
    [
      "Drop Index" ++ case rand:uniform(3) rem 3 of
        1 -> " " ++ lists:nth(rand:uniform(Index_Name_Length), Index_Name);
            ...
            [
              ...
              case rand:uniform(2) rem 2 of
                1 ->
                  " "
                  ++
                  lists:nth(rand:uniform(Drop_Index_Extensions_Length), Drop_Index_Extensions);

                _ -> []
              end
            ]
          )
      end || _ <- lists:seq(1, ?MAX_STATEMENT_SIMPLE * 2)
    ],
  store_code(Rule, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(manipulative_statement, Code, ?MAX_STATEMENT_SIMPLE, true),
  store_code(sql, Code, ?MAX_STATEMENT_SIMPLE, true),
    ...
    [
      lists:append(
        [
          ...
          lists:nth(rand:uniform(Materialized_View_Name_Length), Materialized_View_Name),
          case rand:uniform(2) rem 2 of
            1 -> " Preserve Table";
            _ -> []
          end
...
create_code(drop_procedure_def = Rule) ->
  ?CREATE_CODE_START,
  [{procedure_name, Procedure_Name}] = ets:lookup(?CODE_TEMPLATES, procedure_name),
  ...
  Code =
    [
      lists:append(
        [
          "Drop Profile ",
          lists:nth(rand:uniform(Profile_Name_Length), Profile_Name),
          case rand:uniform(2) rem 2 of
            1 -> " Cascade";
            _ -> []
          end
...
create_code(drop_sequence_def = Rule) ->
  ...
  [{schema_name, Schema_Name}] = ets:lookup(?CODE_TEMPLATES, schema_name),
  Schema_Name_Length = length(Schema_Name),
  [{sequence_name, Sequence_Name}] = ets:lookup(?CODE_TEMPLATES, sequence_name),
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            1 ->
              " " ++ lists:nth(rand:uniform(Drop_Table_Extensions_Length), Drop_Table_Extensions);
...
create_code(drop_table_extensions = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Cascade Constraints", "Cascade Constraints Purge", "Purge"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(drop_tablespace_def = Rule) ->
  ...
  [{tablespace_name, Tablespace_Name}] = ets:lookup(?CODE_TEMPLATES, tablespace_name),
  ...
  Code =
    [
      lists:append(
        [
          "Drop Tablespace ",
          lists:nth(rand:uniform(Tablespace_Name_Length), Tablespace_Name),
          case rand:uniform(2) rem 2 of
            1 ->
              " "
              ++
              lists:nth(rand:uniform(Drop_Tablespace_Extensions_Length), Drop_Tablespace_Extensions);
...
create_code(extra = Rule) ->
  ?CREATE_CODE_START,
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Code_1 = [re:replace(N, "ident", "_extra_", [{return, list}]) || N <- Name],
  Code = [re:replace(N, "IDENT", "_EXTRA_", [{return, list}]) ++ ";" || N <- Code_1],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
  ...
  Code =
    lists:append(
      [
        ...
        [
          case rand:uniform(5) rem 5 of
            1 -> lists:append(["(", lists:nth(rand:uniform(Fun_Arg_Length), Fun_Arg), ")"]);
            ...
            3 ->
              case rand:uniform(2) rem 2 of
                1 -> "+";
                _ -> "-"
              end ++ case rand:uniform(2) rem 2 of
                1 -> bracket_query_spec(lists:nth(rand:uniform(Fun_Arg_Length), Fun_Arg));
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ -> bracket_query_spec(lists:nth(rand:uniform(Fun_Arg_Length), Fun_Arg))
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(fun_args_named = Rule) ->
  ?CREATE_CODE_START,
  [{fun_arg_named, Fun_Arg_Named}] = ets:lookup(?CODE_TEMPLATES, fun_arg_named),
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              bracket_query_spec(lists:nth(rand:uniform(Fun_Arg_Named_Length), Fun_Arg_Named)),
              ",",
              bracket_query_spec(lists:nth(rand:uniform(Fun_Arg_Named_Length), Fun_Arg_Named))
            ]
          );

        _ -> bracket_query_spec(lists:nth(rand:uniform(Fun_Arg_Named_Length), Fun_Arg_Named))
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(function_ref = Rule) ->
  ...
  [{fun_args_named, Fun_Args_Named}] = ets:lookup(?CODE_TEMPLATES, fun_args_named),
      ...
      case rand:uniform(2) rem 2 of
        1 ->
          case rand:uniform(6) rem 6 of
            ...
            2 ->
              lists:append(
                [
                  ...
                  " (",
                  case rand:uniform(2) rem 2 of
                    1 -> lists:nth(rand:uniform(Fun_Args_Length), Fun_Args);
                    _ -> lists:nth(rand:uniform(Fun_Args_Named_Length), Fun_Args_Named)
                  end,
                  ")"
                ]
              );
            ...
            5 -> lists:append([lists:nth(rand:uniform(Identifier_Length), Identifier), " ()"]);
        ...
        _ ->
          lists:nth(rand:uniform(Funs_Length), Funs) ++ case rand:uniform(20) rem 20 of
            1 -> [];
            ...
            _ ->
              lists:append(
                [
                  ...
                  case rand:uniform(6) rem 6 of
                    1 -> lists:nth(rand:uniform(Fun_Args_Length), Fun_Args);
                    2 -> lists:nth(rand:uniform(Fun_Args_Named_Length), Fun_Args_Named);
                    3 -> "*";
                    4 -> "Distinct " ++ lists:nth(rand:uniform(Column_Ref_Length), Column_Ref);
...
create_code(grant_def = Rule) ->
  ...
  Code =
    [
      "Grant" ++ case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              " ",
              case rand:uniform(4) rem 4 of
                1 -> "All Privileges ";
                _ -> lists:nth(rand:uniform(Object_Privilege_List_Length), Object_Privilege_List)
              end,
              " ",
              lists:nth(rand:uniform(On_Obj_Clause_Length), On_Obj_Clause),
              " To ",
              case rand:uniform(4) rem 4 of
                1 -> lists:nth(rand:uniform(Grantee_Identified_By_Length), Grantee_Identified_By);
        ...
        _ ->
          lists:append(
            [
              ...
              case rand:uniform(4) rem 4 of
                1 -> "All Privileges ";
                _ -> lists:nth(rand:uniform(System_Privilege_List_Length), System_Privilege_List)
              end,
              " To ",
              case rand:uniform(4) rem 4 of
                1 -> lists:nth(rand:uniform(Grantee_Identified_By_Length), Grantee_Identified_By);
...
create_code(identifier = Rule) ->
  ?CREATE_CODE_START,
  [{identifier_keyword, Identifier_Keyword}] = ets:lookup(?CODE_TEMPLATES, identifier_keyword),
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_column", [{return, list}]),
        "IDENT",
        "IDENT_COLUMN",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_column_ref", [{return, list}]),
        "IDENT",
        "IDENT_COLUMN_REF",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_context", [{return, list}]),
        "IDENT",
        "IDENT_CONTEXT",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_directory", [{return, list}]),
        "IDENT",
        "IDENT_DIRECTORY",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_from_column", [{return, list}]),
        "IDENT",
        "IDENT_FROM_COLUMN",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_index", [{return, list}]),
        "IDENT",
        "IDENT_INDEX",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_package", [{return, list}]),
        "IDENT",
        "IDENT_PACKAGE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_procedure", [{return, list}]),
        "IDENT",
        "IDENT_PROCEDURE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_profile", [{return, list}]),
        "IDENT",
        "IDENT_PROFILE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_role", [{return, list}]),
        "IDENT",
        "IDENT_ROLE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_scalar_exp", [{return, list}]),
        "IDENT",
        "IDENT_SCALAR_EXP",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_schema", [{return, list}]),
        "IDENT",
        "IDENT_SCHEMA",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_sequence", [{return, list}]),
        "IDENT",
        "IDENT_SEQUENCE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_synonym", [{return, list}]),
        "IDENT",
        "IDENT_SYNONYM",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_table", [{return, list}]),
        "IDENT",
        "IDENT_TABLE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_target", [{return, list}]),
        "IDENT",
        "IDENT_TARGET",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_trigger", [{return, list}]),
        "IDENT",
        "IDENT_TRIGGER",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_type", [{return, list}]),
        "IDENT",
        "IDENT_TYPE",
        [{return, list}]
      )
      ...
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            ...
            _ -> lists:nth(rand:uniform(Scalar_Exp_Commalist_Length), Scalar_Exp_Commalist)
          end,
          ")"
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
              ...
              lists:append(
                [
                  ...
                  case rand:uniform(3) rem 3 of
                          ...
                          case rand:uniform(2) rem 2 of
                            1 ->
                              lists:append(
                                [
                                  lists:nth(
                                    rand:uniform(Scalar_Opt_As_Exp_Length),
                                    Scalar_Opt_As_Exp
                                  ),
                                  ",",
                                  lists:nth(
                    ...
                    _ -> " " ++ lists:nth(rand:uniform(Query_Spec_Length), Query_Spec)
                  end,
                  case rand:uniform(2) rem 2 of
                    1 -> " " ++ lists:nth(rand:uniform(Returning_Length), Returning);
                    _ -> []
      ...
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            ...
            _ -> lists:nth(rand:uniform(Join_Length), Join)
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
...
create_code(join_on_or_using_clause = Rule) ->
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        1 -> "On " ++ lists:nth(rand:uniform(Search_Condition_Length), Search_Condition);
    ...
    [
      lists:append(
        [
          ...
          bracket_query_spec(lists:nth(rand:uniform(Scalar_Exp_Length), Scalar_Exp)),
          case rand:uniform(2) rem 2 of
            1 -> " Escape " ++ lists:nth(rand:uniform(Atom_Length), Atom);
            _ -> []
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
...
create_code(name = Rule) ->
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_function", [{return, list}]),
        "IDENT",
        "IDENT_FUNCTION",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_sys_priv", [{return, list}]),
        "IDENT",
        "IDENT_SYS_PRIV",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "ident", "ident_tbl_type", [{return, list}]),
        "IDENT",
        "IDENT_TBL_TYPE",
        [{return, list}]
      )
...
create_code(object_privilege = Rule) ->
  ?CREATE_CODE_START,
  Code = ["All", "Alter", "Delete", "Execute", "Index", "Insert", "References", "Select", "Update"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(object_with_grant_option = Rule) ->
  ?CREATE_CODE_START,
  Code = ["With Grant Option", "With Hierarchy Option"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(object_with_revoke_option = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Cascade Constraints", "Force"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(on_obj_clause = Rule) ->
  ...
  Code =
    [
      "On" ++ case rand:uniform(3) rem 3 of
        1 -> " " ++ lists:nth(rand:uniform(Table_Length), Table);
        _ -> " Directory " ++ lists:nth(rand:uniform(Identifier_Length), Identifier)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(order_by_clause = Rule) ->
  ...
  Code =
    [
      "Order By " ++ case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
        ...
        _ -> lists:nth(rand:uniform(Ordering_Spec_Length), Ordering_Spec)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(ordering_spec = Rule) ->
  ...
  Code =
    [
      bracket_query_spec(lists:nth(rand:uniform(Scalar_Exp_Length), Scalar_Exp)) ++ case
      rand:uniform(3) rem 3 of
        1 -> " Asc";
        2 -> " Desc";
        _ -> []
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
...
create_code(outer_join_type = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Full", "Full Outer", "Left", "Left Outer", "Right", "Right Outer"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(parameter = Rule) ->
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_from_column", [{return, list}]),
        "PAR",
        "PAR_FROM_COLUMN",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_join_ref", [{return, list}]),
        "PAR",
        "PAR_JOIN_REF",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_table", [{return, list}]),
        "PAR",
        "PAR_TABLE",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_table_alias", [{return, list}]),
        "PAR",
        "PAR_TABLE_ALIAS",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_table_dblink", [{return, list}]),
        "PAR",
        "PAR_TABLE_DBLINK",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "par", "par_table_ref", [{return, list}]),
        "PAR",
        "PAR_TABLE_REF",
        [{return, list}]
      )
...
create_code(parameter_ref = Rule) ->
  ...
  Code =
    [
      lists:nth(rand:uniform(Parameter_Length), Parameter) ++ case rand:uniform(3) rem 3 of
        1 -> " Indicator " ++ lists:nth(rand:uniform(Parameter_Length), Parameter);
        2 -> " " ++ lists:nth(rand:uniform(Parameter_Length), Parameter);
        _ -> []
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ...
  Code =
    [
      lists:append(
        [
          "Begin ",
          lists:nth(rand:uniform(Statement_Pragma_List_Length), Statement_Pragma_List),
          " End;"
        ]
      )
      || _ <- lists:seq(1, ?MAX_STATEMENT_COMPLEX * 2)
    ...
    [
      lists:append(
        [
          ...
          " Connect Through",
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Db_User_Proxy_Length), Db_User_Proxy);
            _ -> " Enterprise Users"
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ...
    Query_Exp ++ [
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            1 ->
              " Union" ++ case rand:uniform(2) rem 2 of
                1 -> " All";
                _ -> []
              end;
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            1 -> " Into " ++ lists:nth(rand:uniform(Target_Commalist_Length), Target_Commalist);
            _ -> []
          end,
          " ",
...
create_code(query_term = Rule) ->
  ...
  Code =
    [
      lists:append(["(", lists:nth(rand:uniform(Query_Exp_Length), Query_Exp), ")"])
      || _ <- lists:seq(1, ?MAX_STATEMENT_SIMPLE * 2)
    ],
  store_code(Rule, Code, ?MAX_STATEMENT_SIMPLE, false),
  store_code(fun_arg, Code, ?MAX_BASIC, false),
...
create_code(quota = Rule) ->
  ...
  Code =
    [
      case rand:uniform(3) rem 3 of
        1 -> "Quota Unlimited On " ++ lists:nth(rand:uniform(Identifier_Length), Identifier);
    ...
    [
      lists:append(
        [
          ...
          case SubSection of
            [] -> [];
            _ -> lists:append(["        --      subsection:    ", SubSection, LineSep])
          end,
          case SubSubSection of
            [] -> [];
            _ -> lists:append(["        --      subsubsection: ", SubSubSection, LineSep])
          end,
          case Example of
            [] -> [];
            _ -> lists:append(["        --                     ", Example, LineSep])
          end,
          "        -- ---------------------------------------------------------------------",
          LineSep,
      ...
      {Book, Chapter, Section, SubSection, SubSubSection, Example, CodeExample}
      <-
...
create_code(revoke_def = Rule) ->
  ...
  Code =
    [
      "Revoke" ++ case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              " ",
              ...
              case rand:uniform(2) rem 2 of
                1 ->
                  ...
                  ++
                  lists:nth(
                    rand:uniform(Object_With_Revoke_Option_Length),
...
create_code(scalar_exp = Rule) ->
  ...
  Code =
    Scalar_Exp ++ [
      bracket_query_spec(lists:nth(rand:uniform(Scalar_Sub_Exp_Length), Scalar_Sub_Exp)) ++ case
      rand:uniform(2) rem 2 of
        1 -> " || " ++ bracket_query_spec(lists:nth(rand:uniform(Scalar_Exp_Length), Scalar_Exp));
        _ -> []
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ -> lists:nth(rand:uniform(Scalar_Opt_As_Exp_Length), Scalar_Opt_As_Exp)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
      ...
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            ...
            2 -> " " ++ lists:nth(rand:uniform(Schema_Element_Length), Schema_Element);
            _ -> []
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_STATEMENT_COMPLEX * 2)
  ...
  Code_1 =
    Search_Condition ++ [
      case rand:uniform(3) rem 3 of
        ...
        _ -> "Not " ++ lists:nth(rand:uniform(Search_Condition_Length), Search_Condition)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  Code = [lists:append(["(", C, ")"]) || C <- Code_1 ++ Search_Condition],
  store_code(Rule, Code ++ Code_1, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(select_field = Rule) ->
  ...
  Code =
    ["*"] ++ [
      lists:nth(rand:uniform(Case_When_Exp_Length), Case_When_Exp) ++ case rand:uniform(3) rem 3 of
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ -> lists:nth(rand:uniform(Select_Field_Length), Select_Field)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
        ...
        1 ->
          lists:append(
            [
              ...
              ";",
              case rand:uniform(2) rem 2 of
                1 -> lists:nth(rand:uniform(Extra_Length), Extra);
                _ -> []
              end,
              lists:nth(rand:uniform(Sql_Length), Sql),
              ";",
...
create_code(statement_pragma_list = Rule) ->
  ?CREATE_CODE_START,
  [{statement_pragma, Statement_Pragma}] = ets:lookup(?CODE_TEMPLATES, statement_pragma),
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              lists:nth(rand:uniform(Statement_Pragma_Length), Statement_Pragma),
              ";",
              lists:nth(rand:uniform(Statement_Pragma_Length), Statement_Pragma),
              ";"
            ]
        ...
        _ -> lists:nth(rand:uniform(Statement_Pragma_Length), Statement_Pragma) ++ ";"
      end || _ <- lists:seq(1, ?MAX_SQL * 2)
    ],
  store_code(Rule, Code, ?MAX_SQL, true),
  ?CREATE_CODE_END;
...
create_code(string = Rule) ->
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "string", "string_atom", [{return, list}]),
        "STRING",
        "STRING_ATOM",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "string", "string_fun_arg", [{return, list}]),
        "STRING",
        "STRING_FUN_ARG",
        [{return, list}]
      )
  ...
  store_code(
    ...
    [
      re:replace(
        re:replace(C, "string", "string_literal", [{return, list}]),
        "STRING",
        "STRING_LITERAL",
        [{return, list}]
      )
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ -> lists:nth(rand:uniform(System_Privilege_Length), System_Privilege)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(system_with_grant_option = Rule) ->
  ?CREATE_CODE_START,
  Code = ["With Admin Option", "With Delegate Option"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        1 -> lists:nth(rand:uniform(Schema_Name_Length), Schema_Name) ++ ".";
        _ -> []
      end ++ lists:nth(rand:uniform(Table_Name_Length), Table_Name) ||
      _
      <-
      lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  store_code(from_column, Code, ?MAX_BASIC, false),
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            1 -> lists:nth(rand:uniform(Column_Ref_Length), Column_Ref);
            2 -> lists:nth(rand:uniform(Function_Ref_Length), Function_Ref);
...
create_code(table_constraint_def = Rule) ->
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(["Constraint ", lists:nth(rand:uniform(Identifier_Length), Identifier), " "]);

        _ -> []
      end ++ case rand:uniform(4) rem 4 of
        1 ->
          lists:append(
            ["Unique (", lists:nth(rand:uniform(Column_Commalist_Length), Column_Commalist), ")"]
          );
      ...
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            ...
            _ -> lists:nth(rand:uniform(From_Column_Length), From_Column)
          end,
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Where_Clause_Length), Where_Clause);
            _ -> []
          end,
          case rand:uniform(2) rem 2 of
            1 ->
          ...
          case rand:uniform(2) rem 2 of
            1 ->
              " Group By "
              ++
              lists:nth(rand:uniform(Column_Ref_Commalist_Length), Column_Ref_Commalist);

            _ -> []
          end,
          case rand:uniform(2) rem 2 of
            1 -> " Having " ++ lists:nth(rand:uniform(Search_Condition_Length), Search_Condition);
            _ -> []
          end,
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Order_By_Clause_Length), Order_By_Clause);
            _ -> []
          end
        ]
      ) || _ <- lists:seq(1, ?MAX_STATEMENT_SIMPLE * 2)
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(3) rem 3 of
            1 -> " Where Current Of " ++ lists:nth(rand:uniform(Cursor_Length), Cursor);
            2 -> " " ++ lists:nth(rand:uniform(Where_Clause_Length), Where_Clause);
            _ -> []
          end,
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(Returning_Length), Returning);
  ...
  Code =
    [
      case rand:uniform(3) rem 3 of
        ...
        _ -> "Profile " ++ lists:nth(rand:uniform(Identifier_Length), Identifier)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  store_code(spec_item, Code, ?MAX_BASIC, false),
...
create_code(user_role = Rule) ->
  ...
  Role_List_Length = length(Role_List),
  Code =
    ["Default Role All", "Default Role None"] ++ [
      "Default Role" ++ case rand:uniform(2) rem 2 of
        1 ->
          lists:append([" All Except", " ", lists:nth(rand:uniform(Role_List_Length), Role_List)]);
...
create_code(when_action = Rule) ->
  ?CREATE_CODE_START,
  [{identifier, Identifier}] = ets:lookup(?CODE_TEMPLATES, identifier),
  Code = ["Continue"] ++ ["Goto " ++ N || N <- Identifier],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
file_create_ct_all(_Type, _CompleteSemicolon, _CompactedDetailed, []) -> ok;
...
file_create_ct(Type, CompleteSemicolon, CompactedDetailed, Rule) ->
  ...
  FileName =
    lists:append([Type, "_", CompleteSemicolon, "_", CompactedDetailed, "_", RuleString, "_SUITE"]),
  {ok, File, _} = file:path_open([?PATH_CT], FileName ++ ".erl", [write]),
  erlang:display(
    io:format(
      user,
      "~n" ++ ?MODULE_STRING ++ " : final common tests ===> ~12.. B file_name: ~s ",
  ...
  io:format(
    File,
    "~s~n",
    [lists:append(["%%% Description : Test Suite for rule: ", RuleString, "."])]
  ),
  io:format(File, "~s~n", ["%%%"]),
  io:format(
    File,
    ...
    [
      ...
      ++
      lists:flatten(
        io_lib:format("~2..0w.~2..0w.~4..0w", [Current_Day, Current_Month, Current_Year])
      )
    ]
  ),
  io:format(
  ...
  case CodeLength of
    ...
    _ ->
      ...
      case CompactedDetailed of
        "compacted" -> io:format(File, "~s~n", [lists:append(["    test_compacted/1"])]);
  ...
  io:format(
    File,
    "~s~n",
    [lists:append(["        {timetrap, {minutes, ", integer_to_list(?TIMETRAP_MINUTES), "}}"])]
  ),
  io:format(File, "~s~n", ["    ]."]),
  io:format(File, "~s~n", [""]),
  ...
  case CodeLength of
    ...
    _ ->
      case CompactedDetailed of
        "compacted" -> io:format(File, "~s~n", [lists:append(["        test_compacted"])]);
  ...
  case CodeLength of
    ...
    _ ->
      case CompactedDetailed of
        "compacted" -> io:format(File, "~s~n", [lists:append(["test_compacted(_Config) ->"])]);
        _ -> ok
      end,
...
file_write_ct_all(Current, File, Target) when Current == Target ->
...
file_write_ct_export(Current, File, Target) when Current == Target ->
...
file_create_eunit_all(_Type, _CompleteSemicolon, []) -> ok;
...
file_create_eunit(Type, CompleteSemicolon, Rule) ->
  ...
  erlang:display(
    io:format(
      user,
      "~n" ++ ?MODULE_STRING ++ " : final eunit  tests ===> ~12.. B file_name: ~s~n",
...
file_write_eunit(_CompleteSemicolon, File, []) -> file:close(File);
...
store_code(Rule, Code, Max, Strict) ->
  case ?LOGGING of
    true ->
      ...
      erlang:display(
        io:format(
          user,
          "~n" ++ ?MODULE_STRING ++ " : store total_heap   ===> ~12.. B rule: ~s~n",
      ...
      erlang:display(
        io:format(
          user,
          "~n" ++ ?MODULE_STRING ++ " : store Code         ===> ~12.. B rule: ~s~n",
  ...
  case Max == 0 of
      ...
      case ?LOGGING of
        true ->
          erlang:display(
            io:format(
              user,
              "~n" ++ ?MODULE_STRING ++ " : store CodeNew      ===> ~12.. B rule: ~s~n",
              [0, atom_to_list(Rule)]
            )
          );
    ...
    _ ->
      ...
      CodeUniqueLimited =
        case CodeUnique_Length > Max of
          true -> lists:sublist(CodeUniqueSorted, 1, Max);
          _ -> CodeUnique
        end,
      CodeTotal =
        case ets:lookup(?CODE_TEMPLATES, Rule) of
          [{Rule, CodeOld}] ->
            lists:sort(
              ?F_RANDOM,
      ...
      CodeNew =
        case Strict andalso CodeTotal_Length > Max of
          true -> [lists:nth(rand:uniform(CodeTotal_Length), CodeTotal) || _ <- lists:seq(1, Max)];
          _ -> CodeTotal
        end,
  ...
  case ?LOGGING of
    true ->
      erlang:display(
        io:format(
          user,
          "~n" ++ ?MODULE_STRING ++ " : store table size   ===> ~12.. B rule: ~s~n",
