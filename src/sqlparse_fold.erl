...
fold(FType, Fun, Ctx, Lvl, {'alter user', Usr, {spec, Opts}} = ST)
    ...
    RT = {lists:append(["alter user ", binary_to_list(Usr), " ", OptsStr]), NewCtx2},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, _Lvl, {'alter user', [Usr | _] = Users, {Grant, GrantArg}} = ST)
    ...
    RT = {lists:append(["alter user ", string:join([binary_to_list(U) || U <- Users], ","), " ", GrantStr]), NewCtx1},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {as, L, R} = ST)
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {as, L, R, {dblink, Dblink}} = ST)
    when is_binary(R) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
    ...
    RT = {lists:append([Fl, binary_to_list(Dblink), " ", binary_to_list(R)]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {Type, A, B, C} = ST)
    ...
    RT = {lists:append([A1, " between ", B1, " and ", C1]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {'create index', Opts, Idx, Table, Spec, Norm, Filter} = ST) ->
    ...
    {TableStr, NewCtx3} = fold(FType, Fun, NewCtx2, Lvl + 1, {table, Table}),
...
fold(FType, Fun, Ctx, Lvl, {'create table', Table, Fields, Opts} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    {Clms, NewCtx3} = lists:foldl(fun(Clm, {Acc, CtxAcc}) ->
        ...
        {[], NewCtx2},
        Fields),
    {TableStr, NewCtx4} = fold(FType, Fun, NewCtx3, Lvl + 1, {table, Table}),
...
fold(FType, Fun, Ctx, Lvl, {'create view', Table, Columns, QuerySpec} = ST) ->
    ...
    {TableStr, NewCtx1} = fold(FType, Fun, NewCtx, Lvl + 1, {table, Table}),
...
fold(FType, Fun, Ctx, _Lvl, {TS, Tab} = ST)
    ...
    RT = {lists:append([atom_to_list(TS), " ", binary_to_list(Tab)]), NewCtx2},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, _Lvl, {'drop index', Indx, []} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [_Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    RT = {"drop index " ++ binary_to_list(Indx), NewCtx1},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {'drop index', Indx, Table} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
...
fold(FType, Fun, Ctx, Lvl, {'drop table', {tables, Tables}, E, RC, Types} = ST)
    ...
    {TablesList, NewCtx2} = lists:foldl(fun(T, {Acc, CtxAcc}) ->
        ...
        {TNew, _} = fold(FType, Fun, NewCtx, Lvl + 1, {table, T}),
        {Acc ++ [TNew], CtxAcc1}
                                        end,
        {[], NewCtx1},
        Tables),
...
fold(FType, Fun, Ctx, Lvl, {exists, Sql} = ST) ->
    ...
    RT = {lists:append(["exists (", SqlStr, ")"]), NewCtx2},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
    ...
    {FieldsStr, NewCtx1} = lists:foldl(fun(F, {Acc, CtxAcc}) ->
        ...
        case F of
            ...
            {'select', _} ->
                ...
                {Acc ++ [lists:append(["(", SubAcc, ")"])], CtxAcc1};
            Other ->
...
fold(FType, Fun, Ctx, Lvl, {from, Froms} = ST) ->
    ...
    {FrmStr, NewCtx1}
        = lists:foldl(
        fun(F, {Acc, CtxAcc}) ->
            ?debugFmt(?MODULE_STRING ++ ":fold ===> F: ~p~n", [F]),
            case F of
                {select, _} = F ->
                    {FoldFStr, CtxAcc1} = fold(FType, Fun, CtxAcc, Lvl + 1, F),
                    {Acc ++ [lists:append(["(", string:strip(FoldFStr), ")"])], CtxAcc1};
                {T, J} = F when is_binary(T), is_list(J) ->
                    {FoldFStr, CtxAcc1} = fold(FType, Fun, CtxAcc, Lvl + 1, F),
                    {Acc ++ [lists:append(["(", string:strip(FoldFStr), ")"])],
                        CtxAcc1};
                ...
                {{param, _}, J} = F when is_list(J) ->
                    ...
                    {Acc ++ [lists:append(["(", string:strip(FoldFStr), ")"])],
                        CtxAcc1};
                Other ->
                    {SubAcc, CtxAcc1} = fold(FType, Fun, CtxAcc, Lvl + 1, {table, Other}),
                    {Acc ++ [SubAcc], CtxAcc1}
            end
        end, {[], NewCtx},
        [case Frm of
...
fold(FType, Fun, Ctx, Lvl, {'$from_as', A, B} = ST)
    ...
    {AStr, NewCtx1}
        = case A of
              ...
              A ->
                  ...
                  {lists:append(["(", string:strip(A0), ")"]), NCtx}
          end,
    ...
    RT = {lists:append([AStr, " ", binary_to_list(B)]), NewCtx3},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {grant, Objs, {OnTyp, On}, {'to', Tos}, Opts} = ST)
    ...
    {OnNew, NewCtx3} = fold(FType, Fun, NewCtx2, Lvl + 1, {table, On}),
...
fold(FType, Fun, Ctx, Lvl, {'hierarchical query', {Part1, Part2}} = ST) ->
    ...
    RT = {lists:append([Part1Str, " ", Part2Str]), NewCtx3},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {'in', L, R} = ST)
    ...
    RT = {lists:append([binary_to_list(L), " in ", case lists:prefix("(", RStr) of
                                                        true -> RStr;
                                                        _ ->
                                                            lists:append(["(", RStr, ")"])
                                                    end]), NewCtx3},
...
fold(FType, Fun, Ctx, Lvl, {insert, Table, {}, {}, Return} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {insert, Table, {cols, Columns}, {values, Values}, Return} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {insert, Table, {cols, Columns}, {select, _} = SubQuery, Return} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
...
fold(FType, Fun, Ctx, Lvl, {Op, A} = ST)
    ...
    {Str, NewCtx3} = case A of
                         A when is_binary(A) ->
                             ...
                             {lists:append([atom_to_list(Op), " (", binary_to_list(A), ")"]), NewCtx2};
                         A ->
                             {As, NewCtx2} = fold(FType, Fun, NewCtx1, Lvl + 1, A),
                             {lists:append([atom_to_list(Op), " (", As, ")"])
                                 , NewCtx2}
                     end,
    NewCtx4 = case FType of
                  top_down -> NewCtx3;
    ...
    {OrderByStr, NewCtx1} = lists:foldl(fun(F, {Acc, CtxAcc}) ->
        ...
        case F of
            {O, Op} when is_binary(O), is_binary(Op) ->
                ...
                {Acc ++ [string:strip(lists:append([binary_to_list(O), " ", binary_to_list(Op)]))], CtxAcc2};
            {{select, _} = O, Op} when is_binary(Op) ->
                {Os, CtxAcc1} = fold(FType, Fun, CtxAcc, Lvl + 1, O),
                CtxAcc2 = Fun(Op, CtxAcc1),
                {Acc ++ [string:strip(lists:append(["(", Os, ") ", binary_to_list(Op)]))], CtxAcc2};
...
fold(FType, Fun, Ctx, Lvl, {prior, Field} = ST) ->
    ...
    RT = {lists:append(["prior ", case string:sub_string(FieldsStr, 1, 7) == "select " of
...
fold(FType, Fun, Ctx, _Lvl, {profile, Profile} = ST) ->
    ...
    RT = {lists:append(["profile ", binary_to_list(Profile)]), NewCtx2},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {ref, {Table, Value2}} = ST)
    when is_list(Value2) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {ref, Table} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
...
fold(FType, Fun, Ctx, _Lvl, 'default role all' = ST) ->
    ...
    RT = {lists:append([atom_to_list(ST)]), NewCtx1},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {table, {as, Table, Alias, {dblink, Dblink}}} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    RT = {lists:append([TableStr, binary_to_list(Dblink), " ", binary_to_list(Alias)]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {table, {as, Table, Alias}} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    RT = {lists:append([TableStr, " ", binary_to_list(Alias)]), NewCtx3},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {table, {param, _} = Table} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
             end,
    {TableStr, NewCtx1} = fold(FType, Fun, NewCtx, Lvl + 1, Table),
    NewCtx2 = case FType of
                  top_down -> NewCtx1;
                  bottom_up -> Fun(ST, NewCtx1)
              end,
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {table, {Table, {dblink, Dblink}}} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    RT = {TableStr ++ binary_to_list(Dblink), NewCtx3},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {table, Table} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
...
fold(FType, Fun, Ctx, Lvl, {'truncate table', Table, Mvl, Storage} = ST) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
    ...
    RT = {lists:append([
        ...
        case Mvl of
            ...
            {'materialized view log', T} ->
                lists:append([atom_to_list(T), " materialized view log "])
        end,
        case Storage of
            {} -> [];
            {'storage', T} -> lists:append([atom_to_list(T), " storage"])
        end
    ]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, Lvl, {Op, L, R} = ST)
    ...
    RT = {lists:append([Fl, " ", atom_to_list(Op), " ", Fr]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {Op, L, R} = ST)
    ...
    RT = {lists:append([binary_to_list(L), " ", atom_to_list(Op), " ", Fr]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, Lvl, {Op, L, R} = ST)
    ...
    RT = {lists:append([Fl, " ", atom_to_list(Op), " ", binary_to_list(R)]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, _Lvl, {Op, L, R} = ST)
    ...
    RT = {lists:append([binary_to_list(L), " ", atom_to_list(Op), " ", binary_to_list(R)]), NewCtx4},
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
...
fold(FType, Fun, Ctx, _Lvl, ST) when is_atom(ST) ->
    ...
    ?debugFmt(?MODULE_STRING ++ ":fold ===> ~n RT: ~p~n", [RT]),
    RT;
fold(FType, Fun, Ctx, _Lvl, {Table, {dblink, Dblink}} = ST) when is_binary(Table) ->
    ?debugFmt(?MODULE_STRING ++ ":fold ===> Start ~p~n ST: ~p~n", [_Lvl, ST]),
    NewCtx = case FType of
                 top_down -> Fun(ST, Ctx);
                 bottom_up -> Ctx
...
columns_join([], _Separator, Result) ->
    ?debugFmt(?MODULE_STRING ++ ":columns_join ===>~n Result: ~p~n", [Result]),
    Result;
columns_join([Head | Tail], Separator, Result) ->
    ?debugFmt(?MODULE_STRING ++ ":columns_join ===>~n Head ~p~n Result: ~p~n", [Head, Result]),
